...
#include "Preferences/PreferencesCategoryItemT.hpp"
...
struct PreferencesDialog2::PreferencesDialog2Private
{
public:
    inline bool hasUnsavedChanges() const {
        return false/**settings.actual != *settings.copy*/;
    }
    inline void restoreSettings() {
        // *settings.actual = *settings.copy;
    }
...
public: /* Logic */
    struct {
        ...
        /// @brief Settings copy on latest show event.
        ApplicationSettings *copy = nullptr;
    } settings;
...
PreferencesDialog2::PreferencesDialog2(ApplicationSettings *settings, QWidget *parent)
    : QDialog(parent, Qt::Tool),
      p(new PreferencesDialog2Private)
{
    p->settings.copy = new ApplicationSettings(this);
    ...
    p->category.model->addCategory(new BehaviorCategoryItem);
    p->category.model->addCategory(new PreferencesCategoryItemT<QLabel>("Test", "://icons/findReplace.png"));
    ...
    connect(p->category.listView->selectionModel(), &QItemSelectionModel::currentRowChanged,
            this,                                   &PreferencesDialog2::onCategoryChanged);
...
PreferencesDialog2::~PreferencesDialog2()
{
    delete p;
}

void PreferencesDialog2::onCategoryChanged(const QModelIndex &index)
{
    ...
    p->content.viewport->setWidget(item->view());
}

void PreferencesDialog2::onOkClicked()
{
    accept();
}

void PreferencesDialog2::onCancelClicked()
{
...
void PreferencesDialog2::onResetClicked()
{
    p->restoreSettings();
}
